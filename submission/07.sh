...
vout=$(echo "$decoded_tx" | jq '.vout | length - 1')
...
raw_unsigned_tx=$(bitcoin-cli -regtest createrawtransaction \
    "[{\"txid\": \"$txid\", \"vout\": $vout}]" \
    "{\"$recipient_address\": $(echo "$amount_to_send / 100000000" | bc -l), \"$change_address\": $(echo "$change_amount / 100000000" | bc -l)}")


echo "$raw_unsigned_tx"
